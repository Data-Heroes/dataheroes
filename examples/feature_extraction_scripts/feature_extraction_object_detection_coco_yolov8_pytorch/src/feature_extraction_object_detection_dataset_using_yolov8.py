...
parser.add_argument(
    "--root-dir",
    required=True,
    help="Absolute or relative path to the root directory of the dataset.",
)
parser.add_argument(
    "--export-dir",
    required=True,
...
parser.add_argument(
    ...
    default=None,
    help="Absolute or relative path to the YOLOv8 model.",
)
parser.add_argument(
...
def build_yolo_model(model_param_location: Path):
    """Builds a YOLOv8 model pretrained on COCO, and transforms it for embedded features extraction
    
    Args:
        model_param_location (Path): path to the YOLOv8 model
    """

    class MyYOLOv8IdentityLayer(torch.nn.Module):
        def __init__(self, original_layer, overwrite_f = None, *args: Any, **kwargs: Any) -> None:
            super().__init__()
    ...
    # Load model
    model : Model = YOLO(model_param_location).model
    # Replace all layers past layer no. 4 with Identities (also replace the m.f link for each layer
    ...
    while True:
        ...
        if layer.i == 4:
            ...
            model.model[current_model_index] = MyYOLOv8IdentityLayer(model.model[current_model_index], overwrite_f=-1)
    model.eval()
    model.to(device)
...
def extract_features_from_dataset(
    ...
    Args:
        ...
        model (DetectionModel): the YOLOv8 model
...
def main():
    ...
    if args.model_param_location is None:
        args.model_param_location = Path('yolov8n.pt')
    else:
        args.model_param_location = Path(args.model_param_location)

    # Build the pretrained model
    yolov8_model, yolov8_device = build_yolo_model(model_param_location=args.model_param_location)
...
if __name__ == "__main__":
    ...
    from aws_utils import shutdown_current_aws_instance
    # shutdown_current_aws_instance()
